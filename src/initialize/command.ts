...
export type ActionParams = Omit<ConfigData, "kotor"> & {
  kotor: string
  k1?: boolean
  k2?: boolean
...
export const initializeCommand: InitializeCommand = new Command()
  .name("init")
  ...
  .action(async (options, command) => {
    ...
    // set config and run
    config.data = options
    const desktopSymlink = options.desktopSymlinkName ?? `kotor${options.kotor}`
    await new Initialize(command, { ...options, config, desktopSymlink }).run()
  })
