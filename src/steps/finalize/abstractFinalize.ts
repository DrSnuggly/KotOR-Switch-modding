...
export abstract class AbstractFinalize {
  ...
  // record of file: checksum
  abstract switchFiles: Record<string, string>
  // record of extension: sub-folder
  abstract overrideFileTypeMap: Record<string, string>

  constructor(
    readonly command: Command<[], OptionValues>,
    { config, force, backup, forceBackup, restoreBackup }: FinalizeParams
  ) {
    this.config = config
  ...
  async run() {
    ...
    // move files in order of specificity
    await this.moveExactROMFileMatches()
    await this.checkAndMoveTextures()
    await this.moveLocalizedFiles()
    for (const [extension, subFolder] of Object.entries(
      this.overrideFileTypeMap
    )) {
      this.moveOverrideFileType(extension, subFolder)
    }
  ...
  abstract moveLocalizedFiles(): void | Promise<void>
  ...
  async moveExactROMFileMatches() {
    ...
    for await (const filePath of gameFiles) {
      ...
      this.fsh.tryFileSystemOperation(() => {
        ...
        for (const line of Object.keys(this.switchFiles)) {
  ...
  async checkAndMoveTextures() {
    ...
    await this.fsh.tryFileSystemOperation(async () => {
      const overrideTextures = Object.keys(this.switchFiles)
      ...
      for await (const filePath of gameFiles) {
        ...
        if (
          ...
          for (const tpcLine of Object.keys(this.switchFiles)) {
            // use RegExp to case-insensitively match the end of the line
            // use / as path separator to avoid matching filenames that are
            // substrings of other file names
    ...
    // calculate warning message steps
...
4. Re-run the finalize command.`
  ...
  moveOverrideFileType(extension: string, targetSubFolder: string) {
