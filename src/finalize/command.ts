...
import { FinalizeK1 } from "~/finalize/k1"
import { FinalizeK2 } from "~/finalize/k2"
...
export const finalizeCommand: FinalizeCommand = new Command()
  .name("finalize")
  .summary(
  ...
  .action(async (options, command) => {
    ...
    const finalizeClass = config.kotor === 1 ? FinalizeK1 : FinalizeK2
    await new finalizeClass(command, { ...options, config }).run()
  })
